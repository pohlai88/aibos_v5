...
export default function EditEmployeePage() {
  ...
  const fetchEmployee = async () => {
    try {
      ...
      setFormData({
        ...
        has_recovery_email: data.has_recovery_email || false,
  ...
  const validateForm = () => {
    ...
    if (formData.user_type === 'employee') {
      if (!formData.recovery_email.trim()) {
        ...
        newErrors.recovery_email = 'Please enter a valid recovery email address'
      }

      if (formData.primary_email === formData.recovery_email) {
        newErrors.recovery_email = 'Recovery email must be different from primary email'
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const employeeData = {
        ...formData,
        has_recovery_email: formData.user_type === 'employee' && !!formData.recovery_email,
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/employees" className="text-indigo-600 hover:text-indigo-900">
            Back to Employees
          </Link>
        </div>
      </div>
          ...
          <div className="bg-white shadow rounded-lg">
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className={`mt-1 block w-full border rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm ${
                        errors.primary_email ? 'border-red-300' : 'border-gray-300'
                      }`}
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`mt-1 block w-full border rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm ${
                        errors.recovery_email ? 'border-red-300' : 'border-gray-300'
                      }`}
                    />
                    {errors.recovery_email && (
                      <p className="mt-1 text-sm text-red-600">{errors.recovery_email}</p>
                    )}
                    {formData.user_type === 'employee' && (
                      <p className="mt-1 text-xs text-gray-500">Required for employees</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-4">
                  <label htmlFor="date_joined" className="block text-sm font-medium text-gray-700">
                    Date Joined
                  </label>
                  <input
                    type="date"
